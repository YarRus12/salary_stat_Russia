...
if __name__ == '__main__':
    ...
    st.sidebar.markdown(
        ...
        <h1 class="sidebar-title"> Этот Сайдбар немного расскажет о результатах реализации проекта </h1> 
        """,
    ...
    st.sidebar.markdown(
        """
        <img src="https://smart-lab.ru/uploads/images/01/73/60/2018/11/14/8e38a4.png" width="100%">
        """,
        unsafe_allow_html=True
    )
    ...
    st.sidebar.info(
        ...
        """
         ...
         - Изменение ВВП и ВВП на душу населения для отрасли Строительства имеет высокую зависимость с изменением уровня заработной платы.
         - Изменение ВВП и ВВП на душу населения для отрасли Образования имеет низкую зависимость с изменением уровня заработной платы.
         - При работе со статистической информацией в указанной сфере следует учитывать как макроэкономические обстоятельства (например, кризис 2008-2009 годов), так и неконтролируемые изменения выборки при смене методологии (например, в 2017 году).

         PS: Вроде получилось неплохо, техническая часть проекта понравилась больше, чем исследовательская. Но это потому что деплой мне нравится больше чем анализ. Это был интересный опыт, спасибо.
